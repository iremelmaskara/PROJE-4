...
int saatdizi[8] = {1,3,10,4,5,10,3,5};  //10 a 48 eklersem 58 olur 58 ise ':'nýn  asci kodu
int timerkesmesi;
...
int main(void)
{
    ...
    LCDkarakteryaz('M'); // Sabit metin baþlangýcý
    LCDkarakteryaz('e');
    LCDkarakteryaz('r');
    LCDkarakteryaz('h');
    LCDkarakteryaz('a');
    ...
    while(1)
    {
         ...
         if(timerkesmesi!=0) //burada 1 sn geçti
         {
...
void setinitsettings()
{
    ...
    //asaðýdaki üç kod portf nin 0 nolu pininin lock özelliðini kapatýyo
    //bu kod olmazsa portf pin0 pin okuma asaðý çalýþmaz
    ...
    TimerConfigure(TIMER0_BASE, TIMER_CFG_A_PERIODIC); //down sayacý
...
//    IntEnable(INT_TIMER0A);
    ...
    //3 yerel interrup ayarlarý //kesmesiz yapacaðýmýz için diðer kodlarý sildik
    ...
    //sadece hangi pinleri uart olarak kullanacaðýz
